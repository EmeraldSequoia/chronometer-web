...
| # | Rule | Value / detail | Driver | Transition |
|---|---|---|---|---|
| W1 | **Main dial centred, FULL height (always)** | Centred (`W/2`, `availV/2`); **`mainR = availV/2`** — the diameter equals the content height, touching the header bottom and footer top (no gap). **Stays full-height across the whole anchor range** (a width constraint may be added later if needed). | size | — |
...
| W3 | **Moon — largest, around the dials & weekday** | Largest circle (× **`awMoonK`** slider, default **1** = max) in the **upper-left**, clearing: the left edge (`halfPad`), the alt/az column (**full gap**, treated as a wall), and the **weekday below** (full gap). Centred in that box. *(Only the weekday sits under the moon (W5), so the floor is just one short line — moon stays large.)* | size + tunable | — |
| W4 | **Map — largest, around the dials** | Largest **2:1** rect (× **`awMapK`** slider, default **1** = max) in the **right region**, clearing the right edge (`halfPad`), header/footer (full gap), and the ecl/eot column (**full gap**). Centred vertically on `cy`. The date sits below it and **never constrains it** (the centred map doesn't reach the bottom row). | size + tunable | — |
| W5 | **Date — split across the display, shared baseline** | `split` mode: the **weekday under the moon (bottom-left)** and the **condensed month-day/year/tz under the map (bottom-right)** — *not* both on one side. Both share **one baseline** (A5-style `dateBaselineBottom`), the deepest weekday descender a full gap above the footer, so "Friday" and "Jun 20" align (no vertical offset). The shared unit is set by the wider piece (the condensed date) fitting the right region (cap `UNIT_MAX = 72`); the weekday then fits the left region. The weekday floors the moon (W3); the date never floors the map (W4). | size | — |
...
*Verified (harness, canon 1920×540, defaults): main dial centred (960,238), **D = 2·mainR = 476 = availV** (full content height); outer dials kept at er=101 in symmetric columns (x=621/1299), full gap between each pair. Date `split` **across the display** — weekday under the moon (cx=248) + condensed date under the map (cx=1672), **both on the same baseline** (dateCY − date2CY = 0). Moon r=132 (floored by the weekday only). Map 448×224 centred on cy; its bottom (350) clears the date ink top (~361), so the date doesn't constrain it. No console errors.*

**Open:** by-eye review pending (the moon has horizontal slack — circle-circle gap to the column dials, instead of the wall approximation, could let it grow further); only canon checked so far.
