...
| L2 | **Height-constrained sizing (text proportional, uncapped)** | Every element is sized to the row height: each circle ⌀ = row height (moon = alt = az = ecl = eot = main dial — all equal **— see open Q below**); map ≈ 2:1 (2·height wide); weekday and condensed date at unit **`u = k · (outer-dial ⌀)`** — a **tunable ratio** `k` (harness slider, default **0.35**), proportional and **uncapped** (`UNIT_MAX` does **not** apply), tracking the dial size at every sample. The 2026/PDT segments stay relative to `u`. | size |
...
*Verified (harness, `?v=13`): A6 canon 1200×120 → chrome dropped, `u = 42` (k=0.35 · ⌀120), all circles ⌀=120, deficit ⇒ outer margins 0 and inner gaps ≈ −67 (overlap). Surplus 2400×120 → outer = halfPad = 30, inner gaps expand evenly. Partial 1960×120 → outer = 13.95 (< halfPad), inner = halfPad. Chrome-drop boundary: 700×352 drops, 814×409 & 1000×503 keep, 150-wide drops.*
