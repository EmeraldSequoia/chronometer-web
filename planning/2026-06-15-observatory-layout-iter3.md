...
| RR2 | **Landscape bottom date: month-day on the bottom line, year/tz on top.** In `split` mode (landscape, date along the bottom), the date2 block draws **year+tz above month-day** so the prominent month-day sits at the same height as the weekday on the left, with the year/tz reading above it. Element rendering is unchanged — only the two lines' vertical order is swapped. | [date-view.ts](../src/observatory/date-view.ts) `drawDateView` (`split`) | ✅ done |
