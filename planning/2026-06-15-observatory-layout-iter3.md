...
| # | Rule | Value / detail | Driver | Transition |
|---|---|---|---|---|
| W1 | **Main dial centred, fills height (always)** | Centred (`W/2`, `availV/2`); `mainR = (availV − 2·gap)/2` — a full gap (`2·halfPad`) to the header top and footer bottom. **Stays full-height across the whole anchor range** (a width constraint may be added later if needed). | size | — |
...
| W3 | **Moon — largest, around the dials & date** | Largest circle (× **`awMoonK`** slider, default **1** = max) in the **upper-left**, clearing: the left edge (`halfPad`), the alt/az column (**full gap**, treated as a wall), and the **date row below** (full gap). Centred in that box. *(The short split date row (W5) is what binds at canon — moon ends up height-limited, but much bigger than with a stacked date.)* | size + tunable | — |
...
| W5 | **Date — split onto one row, lower-left** | `split` mode: **weekday on the left, condensed month-day/year/tz on the right** (A6-style, single line), at the bottom of the left region (bottom a full gap above the footer). The row is sized to fit the left region width (unit capped at `UNIT_MAX = 72`). A single line is far shorter than a 4-line stack, so the moon clears much more room. | size | — |
...
*Verified (harness, canon 1920×540, defaults): main dial centred (960,238) r=190 with a full gap (48) top & bottom; outer dials kept at er=101 in symmetric columns (x=669/1251, Δ to centre 291 each side), full gap (48) between each pair. Date `split` — weekday box left (cx=110) + condensed date box right (cx=370) on one row (height 62) at the bottom. Moon r=135.1 (up from 94.6 with the old stack) — height-limited by the date row, clearing it by exactly a full gap (48); now larger than the outer dials. Map 496×248 clears the right column by a full gap (48), centred on cy. No console errors.*
