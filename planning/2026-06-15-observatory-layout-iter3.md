...
| L4 | **Inner sub-dials: pure proportion (no floor)** | The main dial's three inner sub-dials (utc/solar/sid) use the **canonical, unfloored** proportions — `subR = 73/365·mainR = 0.2·mainR`, `subOffset = 149/365·mainR ≈ 0.408·mainR`. `computeLayout` floors these at 20px / 40px for legibility on small-but-normal dials; A6's dial is small in absolute px (R<100), so the floor would stick and `rescaleMain` would carry an inflated ratio (~0.35) up to the full-size dial, making the sub-dials too big. A6 overrides the floor and re-derives the sub-dial centres from `subOffset` (the same 30°/150°/270° arrangement). | size |
