...
| L2 | **Height-constrained sizing (text proportional, uncapped)** | Every element is sized to the row height: each circle ⌀ = row height (moon = alt = az = ecl = eot = main dial — all equal); map ≈ 2:1 (2·height wide); weekday and condensed date at unit **`u = R` = ½·row-height** (the circle radius) — proportional and **uncapped** (`UNIT_MAX` does **not** apply), but **subordinate to the circle ⌀** (filling the full height made the text look huge; sizing it to the radius keeps it ~half the circle diameter). | size |
...
**Date:** reuses `dateCondensed` (single-line "month-day · year · tz", A4 segment sizes, dots dropped) via the A6 path in `date-view.ts` — weekday and date are each ink-centred in their own box at `dateForceU = u` (= R), **not** dropped to a footer (there is none).
...
*Verified (harness, `?v=13`): A6 canon 1200×120 → chrome dropped, `u = 60` (= R = ½·row-height), all circles ⌀=120, deficit ⇒ outer margins 0 and inner gaps ≈ −67 (overlap). Surplus 2400×120 → outer = halfPad = 30, inner gaps expand evenly. Partial 1960×120 → outer = 13.95 (< halfPad), inner = halfPad. Chrome-drop boundary: 700×352 drops, 814×409 & 1000×503 keep, 150-wide drops.*
