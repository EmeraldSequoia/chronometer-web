...
the geometry tuned in `scripts/historical/harness/layout-harness.html`. See
...
app — see the planning doc §9.6. Tuning is complete, so the harness is kept
under `scripts/historical/harness/` for by-eye checks only. After editing
`anchor-layout.ts`/`layout.ts`/`date-view.ts`, rerun
`./scripts/historical/harness/build-harness.sh` and bump the `?v=N` query in
`layout-harness.html`.
...
(681 KB), generated once at port time by
`scripts/historical/generate-altitude-table.ts`. The `.bin` is committed; the
...
Int16 quantization bounds (see `scripts/historical/compare-altitude-tables.ts`).
...
The globe's own texture is the 360×180 JPEG `globe-360x180.jpg` in the same
directory. It ships as the hand-encoded base64 data URL in
`src/blue-marble-data.ts` (byte-identical to the JPEG); the JPEG is kept as the
source should that data URL ever need regenerating. Two larger renditions
(512×256 and 2048×1024) were downloaded alongside it and never wired up — the
shelved zoomable-picker plan would have used the 2048×1024 one — and live in
`scripts/historical/blue-marble/`.
