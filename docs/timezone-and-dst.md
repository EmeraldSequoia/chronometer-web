...
**Labels.** Intl has no *abbreviation* before the first rule either: `timeZoneName: 'short'` returns the LMT offset itself, with seconds (`GMT-10:31:26`). `src/shared/tz-label.ts` turns any offset-with-seconds abbreviation into `LMT`, tzdata's own abbreviation for local mean time: `tzAbbreviationAt` feeds the Observatory header's zone slot, and `formatZoneLine` builds the `Zone (ABBR) UTC±h:mm` lines under a face, in the time-controller and location popovers and in the Inspector, which keep the exact offset — `Pacific/Honolulu (LMT) UTC-10:31:26`. Seconds-free offset forms pass through: `GMT-10:30` was Honolulu's real standard time in 1900, and `GMT+2` is ICU's en-US name for present-day Vienna (no CET/CEST abbreviation in en-US). A few historical *standard* offsets carried seconds too (Dublin Mean Time, −0:25:21 until 1916) and read LMT — the same mean-time offset adopted as legal time. Pinned by `src/__tests__/tz-label.test.ts`.
