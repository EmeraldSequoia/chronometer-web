...
**Use *the* pool's caches, even for a brief, few-slot need.** The rule is not "any slot-based cache is fine" — it is "use the existing `AstroCachePool`." The pool already provides reusable caches (`finalCache`, `refinementCache`, `midnightCache`) whose invalidation is a single `currentFlag` bump — **O(1), independent of slot count** — so reusing one for a short window (e.g. one part's sub-calculations, or one phase of the tick) is essentially free and is the intended pattern. Reach for a pool cache (re-keyed/invalidated as needed) before ever defining a new cache object of your own. A short-lived push of a pool cache is correct; a freshly-allocated cache or a `Map` is not. (The pool once carried an unused `tempCache`; it was deleted 2026-08-22 — if a genuinely separate simultaneous-dateInterval cache is ever needed, add a named pool cache for it rather than a loose object.)
